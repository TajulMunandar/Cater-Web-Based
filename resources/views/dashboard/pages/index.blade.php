...
@section('content')
    ...
    {{-- Page Header --}}
    <div
        class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3 mb-4 animate-fade-in-up">
        <div>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('pelanggan.baru.create') }}"
                class="btn btn-modern btn-modern-primary d-inline-flex align-items-center gap-2">
                <i class="ti ti-plus" style="font-size:1.1rem;"></i>
                Tambah Pelanggan
            </a>
            <form action="{{ route('dashboard.refresh') }}" method="POST" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-modern btn-modern-outline d-inline-flex align-items-center gap-2"
                    title="Refresh data">
        ...
        @if (count($notifikasi) > 0)
            @foreach ($notifikasi as $notif)
                <div class="alert d-flex align-items-center gap-3 px-4 py-3 mb-3 animate-fade-in"
                    ...
                    <i class="ti ti-{{ $notif['icon'] }}"
                        style="font-size:1.3rem;color:var(--color-{{ $notif['type'] }});"></i>
                    <span class="flex-grow-1" style="font-size:0.9rem;color:var(--color-text);">{{ $notif['pesan'] }}</span>
                    <a href="{{ $notif['url'] }}" class="btn btn-modern btn-modern-outline flex-shrink-0"
                        style="font-size:0.8rem;padding:0.3rem 0.85rem;">Lihat</a>
        ...
        @php
            $cards = [
                [
                    ...
                    'color' => 'primary',
                    'trend' =>
                        $stats['total_pelanggan'] > 0
                            ? 'Aktif: ' . $stats['pelanggan_aktif'] . ' · Non-aktif: ' . $stats['pelanggan_nonaktif']
                            : null,
                    'trendUp' => true,
                ],
                [
                    'label' => 'Aktif',
                    'value' => $stats['pelanggan_aktif'],
                    'icon' => 'user-check',
                    'color' => 'success',
                    'trend' =>
                        $stats['total_pelanggan'] > 0
                            ? round(($stats['pelanggan_aktif'] / $stats['total_pelanggan']) * 100) . '% dari total'
                            : null,
                    'trendUp' => true,
                ],
                [
                    'label' => 'Non-Aktif',
                    'value' => $stats['pelanggan_nonaktif'],
                    'icon' => 'user-x',
                    'color' => 'danger',
                    'trend' =>
                        $stats['total_pelanggan'] > 0
                            ? round(($stats['pelanggan_nonaktif'] / $stats['total_pelanggan']) * 100) . '% dari total'
                            : null,
                    'trendUp' => false,
                ],
                [
                    ...
                    'icon' => 'user-plus',
                    'color' => 'warning',
                    'trend' => $stats['pelanggan_baru_bulan_ini'] > 0 ? now()->translatedFormat('F Y') : null,
                    'trendUp' => true,
                ],
            ];
        @endphp
                ...
                <div class="card-modern card-accent-top acc-{{ $c['color'] }} card-lift h-100">
                    <div class="card-body d-flex align-items-start gap-3" style="padding:1.25rem;">
                        <div class="icon-circle"
                            style="background:var(--color-{{ $c['color'] }}-light);color:var(--color-{{ $c['color'] }});">
                            <i class="ti ti-{{ $c['icon'] }}" style="font-size:1.25rem;"></i>
                        </div>
                        <div class="flex-grow-1 min-w-0">
                            <p class="mb-1"
                                style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-text-muted);">
                                {{ $c['label'] }}</p>
                            <h3 class="kpi-value mb-0" data-count="{{ $c['value'] }}">{{ number_format($c['value']) }}
                            </h3>
                            @if ($c['trend'])
                                <small class="d-inline-flex align-items-center gap-1 mt-1"
                                    ...
                                    @else
                                        <i class="ti ti-arrow-down"
                                            style="font-size:0.75rem;color:var(--color-danger);"></i>
                                    @endif
                                    {{ $c['trend'] }}
                                </small>
                            @endif
                ...
                <div class="d-flex align-items-center justify-content-between px-4 py-3"
                    ...
                    <div class="d-flex gap-2">
                        <span class="badge badge-modern"
                            style="background:var(--color-primary-light);color:var(--color-primary);font-size:0.7rem;">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-4 d-flex flex-column">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                ...
                                <span class="badge badge-modern"
                                    style="background:var(--color-border-light);color:var(--color-text-muted);font-size:0.65rem;font-weight:500;">12
                                    Bulan</span>
                            </div>
                            <div class="flex-grow-1" style="min-height:240px;">
                                ...
                                @else
                                    <div class="d-flex flex-column align-items-center justify-content-center h-100">
                                        ...
                                        <p class="empty-state-text mt-2 mb-0" style="font-size:0.8rem;">Belum cukup data.
                                        </p>
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="col-md-4 d-flex flex-column">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                ...
                                <span class="badge badge-modern"
                                    style="background:var(--color-border-light);color:var(--color-text-muted);font-size:0.65rem;font-weight:500;">
                            ...
                            @if (($pencatatanBulanIni['total_aktif'] ?? 0) > 0)
                                <div class="flex-grow-1 d-flex align-items-center gap-3" style="min-height:240px;">
                                    ...
                                    <div class="d-flex flex-column gap-3">
                                        <div>
                                            <div class="fw-bold"
                                                style="font-size:1.25rem;color:var(--color-text);line-height:1.2;">
                                                {{ $pencatatanBulanIni['tercatat'] }}</div>
                                            <div class="d-flex align-items-center gap-1 mt-1">
                                                <span
                                                    style="width:8px;height:8px;border-radius:2px;background:#059669;flex-shrink:0;"></span>
                                                <small style="color:var(--color-text-muted);font-size:0.75rem;">Tercatat
                                                    ({{ $pencatatanBulanIni['persentase_tercatat'] }}%)</small>
                                            </div>
                                        </div>
                                        <div>
                                            <div class="fw-bold"
                                                style="font-size:1.25rem;color:var(--color-text);line-height:1.2;">
                                                {{ $pencatatanBulanIni['belum_tercatat'] }}</div>
                                            <div class="d-flex align-items-center gap-1 mt-1">
                                                <span
                                                    style="width:8px;height:8px;border-radius:2px;background:#DC2626;flex-shrink:0;"></span>
                                                <small style="color:var(--color-text-muted);font-size:0.75rem;">Belum
                                                    Tercatat ({{ $pencatatanBulanIni['persentase_belum'] }}%)</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="card-modern h-100">
                ...
                <div class="card-body p-4 pb-2">
                    @if(count($distribusiGolongan) > 0)
                        <div class="d-flex flex-column gap-4 golongan-scroll"
                                ...
                                <div>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="fw-medium"
                                            style="font-size:0.85rem;color:var(--color-text);">{{ $g['kode'] ?: $g['golongan'] }}</span>
                                        <span style="font-size:0.8rem;color:var(--color-text-secondary);">{{ $g['total'] }}
                                            · {{ $pct }}%</span>
                                    </div>
                                    <div class="progress-modern">
                                        <div class="progress-bar-graded" role="progressbar"
                                            style="width:{{ $pct }}%;background:linear-gradient(90deg, #2563EB, #0EA5E9);border-radius:100px;"
        ...
        <div class="col-lg-7 animate-fade-in-up stagger-8">
            <div class="card-modern h-100">
                <div class="d-flex align-items-center justify-content-between px-4 py-3"
                    ...
                    <a href="{{ route('pelanggan.baru.index') }}" class="btn btn-modern btn-modern-outline"
                ...
                <div class="p-0">
                                ...
                                <tbody>
                                    @foreach ($aktivitasTerbaru as $i => $p)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="fw-medium"
                                                    style="font-size:0.85rem;color:var(--color-text);">{{ $p->nama }}</span>
                                                <small class="d-block"
                                                    style="font-size:0.7rem;color:var(--color-text-muted);">{{ $p->created_at->diffForHumans() }}</small>
                                            </td>
                                            <td style="font-size:0.85rem;color:var(--color-text-secondary);">
                                                {{ $p->wilayah?->wilayah ?? '-' }}</td>
                                            <td style="font-size:0.85rem;color:var(--color-text-secondary);">
                                                {{ $p->golongan?->nama ?? '-' }}</td>
                                            <td>
                                                @if ($p->status === 'aktif')
                                                    <span class="badge badge-modern"
                                                        style="background:var(--color-success-light);color:var(--color-success);">Aktif</span>
                                                @else
                                                    <span class="badge badge-modern"
                                                        style="background:var(--color-danger-light);color:var(--color-danger);">Non-Aktif</span>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ route('pelanggan.show', $p->id) }}"
                                                    class="btn btn-modern btn-modern-outline"
                                                    style="font-size:0.75rem;padding:0.25rem 0.65rem;">Detail</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    @else
                        <div class="d-flex flex-column align-items-center justify-content-center py-5">
                            ...
                            <a href="{{ route('pelanggan.baru.create') }}"
                                class="btn btn-modern btn-modern-primary btn-sm mt-3">Tambah Pelanggan</a>
                        </div>
                    @endif
                </div>
            </div>
...
@push('head')
    ...
    <style>
        ...
        #mapDashboard .leaflet-control-zoom {
        ...
        #mapDashboard .leaflet-control-zoom a {
            border: none !important;
            color: #374151;
            width: 32px;
            height: 32px;
        ...
        #mapDashboard .leaflet-control-zoom a:hover {
            background: #F3F4F6;
        }

        #mapDashboard .leaflet-tile {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(ctx) {
                                    return ctx.label + ': ' + ctx.parsed + ' pelanggan';
                                }
                            }
                        }
                    }
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(ctx) {
                                    if (ctx.dataIndex === 0) return 'Tercatat: ' + data.tercatat +
                                        ' pelanggan (' + data.persentase_tercatat + '%)';
                                    return 'Belum Tercatat: ' + data.belum_tercatat + ' pelanggan (' + data
                                        .persentase_belum + '%)';
                                }
                            }
                        }
            ...
            fetch('{{ route('dashboard.notifikasi') }}')
                ...
                .then(function(data) {
                    ...
                    container.innerHTML = data.map(function(n) {
                        return '<div class="alert d-flex align-items-center gap-3 px-4 py-3 mb-3 animate-fade-in" style="background:var(--bg-card);border:1px solid var(--color-border);border-radius:var(--radius-md);border-left:4px solid var(--color-' +
                            n.type + ');box-shadow:var(--shadow-sm);">' +
                            '<i class="ti ti-' + n.icon + '" style="font-size:1.3rem;color:var(--color-' + n
                            .type + ');"></i>' +
                            '<span class="flex-grow-1" style="font-size:0.9rem;color:var(--color-text);">' + n
                            .pesan + '</span>' +
                            '<a href="' + n.url +
                            '" class="btn btn-modern btn-modern-outline flex-shrink-0" style="font-size:0.8rem;padding:0.3rem 0.85rem;">Lihat</a>' +
